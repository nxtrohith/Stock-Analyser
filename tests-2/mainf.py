...
def predict_stock_trend(stock_ticker,stock_data, company_name, start_date, end_date, api_key, prediction_type):
    # Ensure a larger date range to calculate technical indicators
    if stock_data is None or stock_data.empty:
        return None, None

    df = stock_data.copy()
    # Only fetch news if needed
    if prediction_type in [1, 3]:
        news_data = fetch_news_data(api_key, stock_data.index.date, company_name)
    else:
        news_data = [None] * len(stock_data)

    df = process_data(stock_data, news_data)
    if df is None or len(df) < 5:
        print("Not enough processed data to continue.")
        return None, None
    ...
    print(f"After technical feature processing, data size: {len(df)} rows")
